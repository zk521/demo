...
var artDialog = function (config, ok, cancel) {
	...
	art.SYS_API = art.SYS_API || [];
     var that_Api = art.SYS_API[config.id];
...
artDialog.fn = artDialog.prototype = {
	...
	content: function (msg,url) {
		var prev, next, parent, display,
			...
			if (isIframe) {
				if (typeof hash == 'string') url += (url.indexOf('?') > -1 ? '&': '?') + 'hash=' + hash;
		...
		if (typeof msg === 'string') {
			...
			if (isIframe) art.SYS_API[that.config.id] = msg.contentWindow
		};
